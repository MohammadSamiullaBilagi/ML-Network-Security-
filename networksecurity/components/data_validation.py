...
class DataValidation:
    ...
    # here comparing the columns from schema and dataframe i get 
    ...
    # def check_missing_numerical_columns(dataframe, required_numerical_columns):
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            # required_numerical_columns = train_dataframe.select_dtypes(include=['number']).columns.tolist()

            # train_ok, missing_train_cols = self.check_missing_numerical_columns(train_dataframe, required_numerical_columns)
            # if not train_ok:
            #     error_message = f"Training dataframe missing numerical columns: {missing_train_cols}"
            #     raise NetworkSecurityException(error_message, sys)

            # Check test DataFrame
            # test_ok, missing_test_cols = self.check_missing_numerical_columns(test_dataframe, required_numerical_columns)
            # if not test_ok:
            #     error_message = f"Testing dataframe missing numerical columns: {missing_test_cols}"
